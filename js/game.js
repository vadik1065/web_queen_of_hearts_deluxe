...
//        this.configFile = './conf/desktop.json';
        ...
        let font1 = new FontFace('Tangodi', 'url(./fonts/tangodi.woff)');
        ...
        let font2 = new FontFace('RobotoBold', 'url(./fonts/roboto-bold.woff)');
        font2.load().then( function( fontFace ) {
            document.fonts.add( fontFace );
        })
        .catch( function(error) {
            Log.error( error );
    ...
    onConfigLoaded( result ) {
        ...
        this.addToLoadItem( this.images.logo );         // логотип игры
        ...
        this.addToLoad( helpDef.payTable.urls );
        this.addToLoad( helpDef.book.urls );
        this.addToLoad( helpDef.symbols.heart.urls );
        this.addToLoad( helpDef.symbols.girl.urls );
        this.addToLoad( helpDef.symbols.cupid.urls );

        let bonusDef = this.bonusDef;                   // картинки для баннера бонус-игры
        this.addToLoadItem( bonusDef.banner );
        this.addToLoad( bonusDef.book.urls );
        this.addToLoad( bonusDef.book.flipPages.urls );
        this.addToLoad( bonusDef.book.flipSymbols.urls );
    ...
    create() {
        ...
        game.bonusBanner = new BonusBanner( game, game.bonusDef );
        game.bonusBanner.addListener( 'animationFinished', game.onBannerAnimationFinished );
    ...
    controlItem( name ) {
        if ( this.controlBox ) {
            return this.controlBox.item( name );
        }
        return null;
    }
    //==========================================================================
...
//        game.showMsgAbove( Tools.formatAmount( game.isBonusProcess ? game.spinWinAmount : winAmount ) + " WIN", true );
//        game.showMsgBelow( "GAMBLE AMOUNT " + Tools.formatAmount( game.totalWinAmount + game.spinWinAmount ) );
        ...
        if ( lineNum === "100" ) {  // выпали скаттер-символы (начало бонус игры)
            game.currentWinLineSound = "origin/bonus_start";
        ...
        if ( fiveSymbolsSound ) {   // есть есть звук пяти символов
            if ( soundFile ) {          // есть звук комбинации символов
                ...
                // Сначала проигрываем звук пяти символов, а затем - звук комбинации
                game.currentWinLineSound = fiveSymbolsSound;
                game.startPlay( fiveSymbolsSound, function(){
                    game.currentWinLineSound = soundFile;
                    game.startPlay( soundFile, lineBox.onNextLine, { self: lineBox } );
                });
                return;
            }
            soundFile = fiveSymbolsSound;
        }
        if ( soundFile ) {
...
//        this.showMsgBelow( "GAMBLE AMOUNT " + Tools.formatAmount( this.totalWinAmount + this.spinWinAmount ) );
    ...
    onBonusFilesLoaded() {
        ...
        if ( this.bonusBanner.isAnimationFinished() ) {
    ...
    showFreeSpinBanner( callback ) {
        this.bonusBanner.showText( "" + this.serverData.bonusGame.winFreeSpin + ' MORE FREE GAMES' );
        setTimeout( ()=>{
    ...
    onExtraLineShown( params ) {
        ...
        game.currentWinLineSound = "origin/countup";
        game.startPlay( game.currentWinLineSound );

        // Подсчитать выигрыш по спину и показать общий выигрыш и выигрыш по спину

        let amount = params.win[ 1 ];
        game.spinWinAmount += amount;
        game.showMsgAbove( Tools.formatAmount( game.spinWinAmount ) + " WIN", true );
//        game.showMsgAbove( Tools.formatAmount( game.totalWinAmount + game.spinWinAmount ) + " WIN", true );
//        game.showMsgBelow( "GAMBLE AMOUNT " + Tools.formatAmount( game.totalWinAmount + game.spinWinAmount ) );
    }
    ...
    onBonusGameFinished() {
        ...
        game.bonusBanner.showText( text );
//        game.showMsgBelow( "GAMBLE AMOUNT " + Tools.formatAmount( game.totalWinAmount ) );
